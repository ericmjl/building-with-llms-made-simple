...
def _():
    ...
    return BaseModel, DocstringBreakdown, Field, List, Optional


@app.cell
def _(DocstringBreakdown, lmb):
    @lmb.prompt("system")
...
@app.cell(hide_code=True)
def _(BaseModel, Field, List, Optional, lmb, mo):
...
@app.cell
def _(DocstringEvaluation):
    from inspect import getsource

    print(getsource(DocstringEvaluation))
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""After cycling through `llama3.1`, `llama3.2`, and finally `phi4`, I found `phi4` to be the one model that was able to follow the examples provided and use sphinx-style docstrings:""")
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""However, we should also ablate the prompt (by switching it back to the basic prompt to make sure that the in-context learning examples were the ones that steered the LLM the right way.""")
    return


@app.cell
def _(DocstringBreakdown, function_request, improved_system_prompt, lmb):
    bot_no_examples = lmb.StructuredBot(
        system_prompt=improved_system_prompt(good_examples=[]),
        pydantic_model=DocstringBreakdown,
    ...
    response_phi4_no_examples = bot_no_examples(function_request)
    print(response_phi4_no_examples.docstring)
    return
