...
def _(mo):
    mo.md(
        r"""Now, create the SimpleBot that is going to be generating the commit message."""
    )
    return
...
def _(mo):
    mo.md(
        r"""It returns a User message (instantiated as a BaseMessage), and has both the `role` and the `content` fields."""
    )
    return
...
def _(mo):
    mo.md(
        r"""Since we have the message formatted properly, we can pass it directly to our commit message bot:"""
    )
    return
...
def _(mo):
    ...
    Let's see how GPT-4o works compared to Llama 3.2.
    """
    )
    return
